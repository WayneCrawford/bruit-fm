...
streams['rot_all'].select(channel='LHZ').plot()
streams['rot_all'].select(channel='LHZ').plot(outfile='plots/AS02.cleaned.z_waveform.png')
...
z_compare.plot(outfile=f'plots/AS02.cleaned.z_compare.png')
sd_compare = SpectralDensity.from_stream(z_compare, inv=inv)
sd_compare.plot(overlay=True, outfile=f'plots/AS02.cleaned.sd_compare.png')
...
sd.plot_one_coherence('*Z', '*H', outfile=f'plots/AS02.cleaned.ZHcoher.png')
...
rf.plot(outfile=f'plots/AS02.cleaned.ZHcompl.png')

# Output compliance
output_compliance(rf, 'AS02.cleaned', max_freq=0.05)